...
// invoice_listDlg.cpp : implementation file
//

#include "stdafx.h"
#include "invoice_list.h"
#include "invoice_listDlg.h"
...
Cinvoice_listDlg::Cinvoice_listDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(Cinvoice_listDlg::IDD, pParent)
	, m_month("")
	, m_year("")
{
...
void Cinvoice_listDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Cinvoice_listDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDOK, &Cinvoice_listDlg::OnBnClickedOk)
END_MESSAGE_MAP()


// Cinvoice_listDlg message handlers

BOOL Cinvoice_listDlg::OnInitDialog()
{
...
HCURSOR Cinvoice_listDlg::OnQueryDragIcon()
{
...
void Cinvoice_listDlg::OnBnClickedOk()
{
	UpdateData(TRUE);
	...
	} else
	{
		...
		// check profit_temp
		decade_set.Open(CRecordset::forwardOnly, L"select * from dbo.sysobjects where name='profit_temp'");
		if (!decade_set.IsEOF()) 
		{
			decade_set.Close();
			decade.ExecuteSQL(L"drop table profit_temp");
		} else
		{
			decade_set.Close();
		}
		decade.ExecuteSQL(L"create table profit_temp(custName varchar(32), custNum varchar(32), currency varchar(32), date varchar(32), invoiceNum varchar(32), dieSale decimal(18,2), surcharge decimal(18,2), fastTrack decimal(18,2), other decimal(18,2), totalSale decimal(18,2), cost decimal(18,2), profit decimal(18,2), rate decimal(18,4))");
		for each(Invoice invoice in vecInvoice)
		{
			query = L"insert into profit_temp values ('";
			...
			query += invoice.cost + L"', '";
			query += invoice.profit + L"', '";
			query += invoice.rate + L"')";
			decade.ExecuteSQL(query);
		}
		// check profit_date_temp
		decade_set.Open(CRecordset::forwardOnly, L"select * from dbo.sysobjects where name='profit_date_temp'");
		if (!decade_set.IsEOF()) 
		{
			decade_set.Close();
			decade.ExecuteSQL(L"drop table profit_date_temp");
		} else
		{
			decade_set.Close();
		}
		decade.ExecuteSQL(L"create table profit_date_temp(plant varchar(32),date varchar(32))");
		query = L"insert into profit_date_temp values ('ETS','";
		query += m_year;
		query += L"/";
		query += m_month;
		query += L"')";
	...
	ShellExecute(NULL, NULL, L"\\\\10.0.0.6\\shopdata\\development\\tiger\\report\\profit.rpt", NULL, NULL, SW_SHOW);
	OnOK();
}
