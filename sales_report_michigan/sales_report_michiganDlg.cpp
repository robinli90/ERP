...
// sales_report_michiganDlg.cpp : implementation file
//

#include "stdafx.h"
#include "sales_report_michigan.h"
#include "sales_report_michiganDlg.h"
...
Csales_report_michiganDlg::Csales_report_michiganDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(Csales_report_michiganDlg::IDD, pParent)
    , m_year(_T(""))
    , m_month(_T(""))
{
...
void Csales_report_michiganDlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(Csales_report_michiganDlg, CDialogEx)
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
    ON_BN_CLICKED(IDOK, &Csales_report_michiganDlg::OnBnClickedOk)
END_MESSAGE_MAP()


// Csales_report_michiganDlg message handlers

BOOL Csales_report_michiganDlg::OnInitDialog()
{
...
void Csales_report_michiganDlg::OnPaint()
{
	if (IsIconic())
	{
...
HCURSOR Csales_report_michiganDlg::OnQueryDragIcon()
{
...
void Csales_report_michiganDlg::OnBnClickedOk()
{
	vector<Inv> invoice;
	UpdateData(TRUE);
	// get record
	...
	} else
	{
		...
		query = L"select a.dhincr as cn, a.dhinv# as inv, a.dhcurr as currency, a.dhidat as date, a.dhbnam as customer_name, a.dhbcs# as customer_id, a.dhtoti as invoice, a.dhpost as posted from cmsdat.oih as a where a.dhplnt='003' and (a.dhidat>='2012-01-01' and a.dhidat<='2012-12-31')";		
	...
	} else
	{
		...
		// check profit_temp
		decade_set.Open(CRecordset::forwardOnly, L"select * from dbo.sysobjects where name='sale_report_michigan_temp'");
		if (!decade_set.IsEOF()) 
		{
			decade_set.Close();
			decade.ExecuteSQL(L"drop table sale_report_michigan_temp");
		} else
		{
			decade_set.Close();
		}
		decade.ExecuteSQL(L"create table sale_report_michigan_temp(CREDIT_NOTE char, INVOICE# varchar(32), CURRENCY varchar(32), DATE varchar(32), CUST_NAME varchar(32), CUST_ID varchar(32), REFERENCE varchar(32), SALE decimal(18,2), DISCOUNT decimal(18,2), FAST_TRACK decimal(18,2), SURCHARGE decimal(18,2), FREIGHT decimal(18,2), TAX decimal(18,2), TOTAL decimal(18,2), POST char)");
		for each(Inv inv in invoice)
		{
			query = L"insert into sale_report_michigan_temp values ('";
