...
{
    public partial class Add_Ring : Form
    {
        DieType dt = DieType.Ring;
        ...
        /// <param name="g"></param>
        /// <param name="s"></param>
        public Add_Ring(Main _parent, Point g = new Point(), Size s = new Size(), Support Ref_Support_ = null)
        {
            ...
            {
                MetricFactor = 25.4;
                dim1.Text = dim2.Text = dim3.Text = "mm";
            }

            if (Ref_Support_ != null)
            {
                Ref_Support = Ref_Support_;
                dt = Ref_Support.DieType;
                addSolidButton.Image = global::ExcoPricingTool.Properties.Resources.save;
                label15.Text = "SAVE";

                // Set current values to text
                string[] name = Ref_Support.DieNo.Split(new string[] { "--" }, StringSplitOptions.None);
                dieNo.Text = name[0];
                if (name.Count() > 1) copy.Text = name[1];

                location.Text = Ref_Support.Location;
                dia.Text = (Ref_Support.Diameter * MetricFactor).ToString();
                thk.Text = (Ref_Support.Thickness * MetricFactor).ToString();
                innerd.Text = (Ref_Support.InnerDiameter * MetricFactor).ToString();

                notes = Ref_Support.Notes.ToString();

                DieCharge_List = Ref_Support.Charge_List;
            }
        }
        ...
        private void Form_Load(object sender, EventArgs e)
        {
            ...
            if (Ref_Support != null)
            {
                Ref_BaseCharge = getDiaThk(Convert.ToDouble(dia.Text), Convert.ToDouble(thk.Text), Convert.ToDouble(innerd.Text)) ?? new baseCharges();
        ...
        {
            Grey_Out();
            if (Ref_BaseCharge != null && Ref_Support == null)
            {
        ...
        {
            if (dieNo.Text.Length > 0 && dia.Text.Length > 0 && thk.Text.Length > 0 && innerd.Text.Length > 0)
            {
            ...
            else
            {
                ...
                {
                    error_message = "Error: Missing Inner Diameter";
                }
        ...
        private void dia_TextChanged(object sender, EventArgs e)
        {
            ...
            if (dia.Text.Length > 0 && thk.Text.Length > 0 && innerd.Text.Length > 0)
            {
                Ref_BaseCharge = getDiaThk(Convert.ToDouble(dia.Text), Convert.ToDouble(thk.Text), Convert.ToDouble(innerd.Text)) ?? new baseCharges();
                Calculate_Die_Price(true);
            }
        ...
        private double Calculate_Die_Price(bool silent = false)
        {
            if (Loaded)
            {
                ...
                try
                {
                    ...
                    if (price == 0 || Ref_BaseCharge.InnerDiameter - 1 > Ref_BaseCharge.Diameter) Error = true;
        ...
        private void innerd_TextChanged(object sender, EventArgs e)
        {
            parent.textBoxTextChanged2(sender, e);
            if (dia.Text.Length > 0 && thk.Text.Length > 0 && innerd.Text.Length > 0)
            {
